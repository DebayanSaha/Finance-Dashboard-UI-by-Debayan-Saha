...
export default function StatisticsChart() {
  ...
  return (
    <div className="bg-white rounded-2xl p-5 w-full h-full flex flex-col
      ...
      {/* Header */}
      <div className="mb-1">
        <span className="text-[22px] font-[font2] text-gray-900 max-sm:text-lg">
          Statistics
        </span>
      </div>
      ...
      {/* ── Donut — fixed size on desktop, scales down on mobile ── */}
      <div className="flex justify-center items-center mb-5">
        {/*
          Desktop: 224×224  |  Tablet: 180×180  |  Mobile: 160×160
          We swap canvas size via a responsive wrapper using CSS scaling.
        */}
        <div className="relative w-[224px] h-[224px] flex-shrink-0
          max-md:w-[180px] max-md:h-[180px]
          max-sm:w-[160px] max-sm:h-[160px]">

          {/* Canvas is fixed at 224px — CSS scales the container */}
          ...
          <div className="absolute inset-0 flex flex-col items-center
            ...
            <div className="flex items-baseline gap-px">
              <span className="text-[24px] font-[font1] text-gray-900 max-sm:text-[18px]">
                ${total.toLocaleString()}
              </span>
              <span className="text-[15px] text-gray-300 max-sm:text-[11px]">.00</span>
            </div>
          ...
          {/* Top-spend badge — repositions on mobile */}
          <div className="absolute -top-1 -right-14 bg-orange-50 border
            border-orange-100 rounded-xl px-2.5 py-2 text-center
            max-sm:-right-10 max-sm:px-1.5 max-sm:py-1.5">
        ...
        <div className="grid grid-cols-2 gap-x-3 gap-y-2.5 max-sm:gap-y-1.5">
          {categories.map(({ name, percentage }, i) => (
            <div key={name} className="flex items-center gap-1.5">
              ...
              />
              <span className="text-[11px] text-gray-500 truncate flex-1 max-sm:text-[10px]">
                {name}
              </span>
              <span
