...
const MonthSelector = ({ selectedMonth, selectedYear, onMonthChange }) => {
  ...
  return (
    <div className="relative" ref={popupRef}>
      {/* Trigger */}
      <div
        className="w-12 h-12 rounded-full border border-[#fbd69e] flex items-center justify-center cursor-pointer"
      ...
      {isOpen && (
        <div
          className="absolute top-14 left-0 z-50 bg-white rounded-xl border border-[#fbd69e] p-4 w-72"
          style={{ boxShadow: "0 8px 32px rgba(255,115,50,0.12)" }}
        >
          {/* Header */}
          <div className="flex items-center justify-between mb-4">
            <button
              className="text-[#ff7332] text-xl px-2 py-1 rounded-full hover:bg-[#fff3ea] cursor-pointer"
              onClick={() => setNavYear((y) => y - 1)}
            >
              ‹
            </button>

            <span className="font-[font2] text-[#8f5b43] text-base ">
              {navYear}
            </span>
            ...
            <button
              ...
              `}
              onClick={() => {
                if (navYear < currentYear) setNavYear((y) => y + 1);
              }}
            >
              ›
            </button>
          ...
          <div className="grid grid-cols-4 gap-2">
            {MONTHS.map((month, index) => {
              const isSelected =
                index === selectedMonth && navYear === selectedYear;
          ...
          {/* Footer */}
          <p className="text-center text-xs text-[#c4a08a] mt-4">
